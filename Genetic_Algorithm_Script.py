...
Mutation_rate = 0.1
Desire_scaler = [ 1, 0, 1, 1, 0] # 5 number with first unnecessary value
...
class Individual:
    def __init__(self):
        ...
        self.fitness = 0
        i = 0
        while i < Desire_scaler.__len__():
            charac = round(random.random(), 3)
            self.character.append(charac)
            i += 1
...
class GeneticAlgorithm:
    ...
    def crossover_individuals(individual1, individual2):
        ...
        for i in range (Desire_scaler.__len__()):
...
def print_all_feature(feature, generation):
    ...
    print("Desire character: ", Desire_scaler )
    print(".........")
    i = 0
...
Population = population(INDIVIDUAL_NUMBER)
Population.get_individuals().sort(key=lambda x:x.get_fitness(), reverse=True)
print_all_feature(Population, 0)
generation_number = 1
while Population.get_individuals()[0].get_fitness() < Desire_scaler.__len__():
